...
byte_t copy(cbytes_t source, bytes_t destination,
            unsigned long sizeof_destination) {
...
// subcopy method
// if the function returns -1 it means that the start_index variable is greater
// than the end_index variable.
byte_t subcopy(cbytes_t src, unsigned int start_index, unsigned int end_index,
               bytes_t dest, unsigned long sizeof_dest) {
  ...
  // Check if the index exceeds the length of the string to be pasted
  // (destination). If it exceeds, update index to the last index of the string
  // to be pasted. if(end_index >= sizeof_dest) end_index = (sizeof_dest - 1);
...
byte_t subcopy_len(cbytes_t src, unsigned int dest_len, bytes_t dest,
                   unsigned long sizeof_dest) {
  return subcopy(src, 0, (dest_len - 1), dest, sizeof_dest);
}

byte_t subcopy_index_len(cbytes_t src, unsigned int start_index,
                         unsigned int dest_len, bytes_t dest,
                         unsigned long sizeof_dest) {
  return subcopy(src, start_index, ((start_index + dest_len) - 1), dest,
                 sizeof_dest);
}
