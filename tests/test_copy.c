...
int test_copy() {
  ...
  if (!equal(name, name_copy)) {
    log_err("Test 1 failed: name ('%s') isn't equal to name_copy ('%s')", name,
            name_copy);
    return 1;
  }

  log_ok("Test 1 passed: name ('%s') is equal to name_copy ('%s')", name,
         name_copy);
  ...
  if (equal(name, name_copy_err)) {
    log_err("Test 2 failed: name ('%s') is equal to name_copy_err ('%s')", name,
            name_copy_err);
    return 1;
  }

  log_ok("Test 2 passed: name ('%s') is not equal to name_copy_err ('%s')",
         name, name_copy_err);
